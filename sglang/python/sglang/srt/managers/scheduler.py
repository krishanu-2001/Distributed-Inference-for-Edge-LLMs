...
class Scheduler(
    ...
    def init_request_dispatcher(self):
        self._request_dispatcher = TypeBasedDispatcher(
            [
                ...
                (ReadRadixTreeReq, self.read_radix_tree),
    ...
    def _unwrap_tree_cache_for_debug(self):
        cache = self.tree_cache
        wrapper_chain = []

        while isinstance(cache, SessionAwareCache):
            wrapper_chain.append(type(cache).__name__)
            cache = cache.inner

        return cache, wrapper_chain

    def _serialize_radix_tree_node(
        self,
        node: Any,
        ...
        segment_key = getattr(node, "key", [])
        if hasattr(segment_key, "token_ids"):
            segment_token_ids = list(segment_key.token_ids)
            extra_key = getattr(segment_key, "extra_key", None)
        else:
            segment_token_ids = list(segment_key) if segment_key is not None else []
            extra_key = None

        value = getattr(node, "value", None)
        if isinstance(value, torch.Tensor):
            kv_indices_preview = value.tolist() if include_values else value[:32].tolist()
            kv_len = int(value.numel())
        elif value is None:
            kv_indices_preview = None
            kv_len = 0
        else:
            kv_indices_preview = list(value) if include_values else list(value[:32])
            kv_len = len(value)
        ...
        return {
            ...
            "evicted": getattr(node, "evicted", False),
            "hit_count": getattr(node, "hit_count", None),
            "priority": getattr(node, "priority", None),
            "num_children": len(children),
            "hash_value": getattr(node, "hash_value", None),
            "last_access_time": getattr(node, "last_access_time", None),
            "creation_time": getattr(node, "creation_time", None),
            "kv_indices_len": kv_len,
            "kv_indices_preview": kv_indices_preview,
            "kv_indices_truncated": (
                False if value is None or include_values else kv_len > len(kv_indices_preview)
            ),
        }

    def _serialize_live_radix_tree(
        self, cache: Any, include_values: bool, max_nodes: int
    ) -> Dict[str, Any]:
        root = getattr(cache, "root_node", None)
        if root is None:
            debug_dump = None
            if hasattr(cache, "debug_dump"):
                try:
                    debug_dump = cache.debug_dump()
                except Exception as exc:
                    debug_dump = f"Unable to capture debug dump: {exc}"
            elif hasattr(cache, "pretty_print"):
                try:
                    buf = io.StringIO()
                    with redirect_stdout(buf):
                        cache.pretty_print()
                    debug_dump = buf.getvalue()
                except Exception as exc:
                    debug_dump = f"Unable to capture pretty_print output: {exc}"

            return {
                "supported": False,
                "reason": (
                    "This cache implementation does not expose a Python-traversable "
                    "root_node. This is expected for some optimized cache variants."
                ),
        ...
        while stack and len(nodes) < max_nodes:
            node, parent_prefix, depth = stack.pop()
            node_key = getattr(node, "key", [])
            if hasattr(node_key, "token_ids"):
                segment_token_ids = list(node_key.token_ids)
            else:
                segment_token_ids = list(node_key) if node_key is not None else []

            prefix_token_ids = parent_prefix + segment_token_ids
            nodes.append(
                self._serialize_radix_tree_node(
                    node=node,
                    prefix_token_ids=prefix_token_ids,
                    depth=depth,
            ...
            ordered_children = sorted(
                children.values(),
                key=lambda child: repr(getattr(child, "key", None)),
                reverse=True,
            )
            for child in ordered_children:
                stack.append((child, prefix_token_ids, depth + 1))

        return {
            "supported": True,
            "total_nodes_returned": len(nodes),
            "truncated": len(stack) > 0,
            "nodes": nodes,
        }

    def read_radix_tree(self, recv_req: ReadRadixTreeReq):
        try:
            cache, wrapper_chain = self._unwrap_tree_cache_for_debug()
            max_nodes = max(1, int(recv_req.max_nodes))
            ...
            payload = {
                ...
                "disable": getattr(cache, "disable", None),
                "evictable_size": (
                    cache.evictable_size() if hasattr(cache, "evictable_size") else None
                ),
                "protected_size": (
                    cache.protected_size() if hasattr(cache, "protected_size") else None
                ),
                "total_size": cache.total_size() if hasattr(cache, "total_size") else None,
            }
            payload.update(
                self._serialize_live_radix_tree(
                    cache=cache,
                    include_values=recv_req.include_values,
                    max_nodes=max_nodes,
                )
            )
        except Exception as exc:
        ...
        return ReadRadixTreeReqOutput(radix_tree=payload)

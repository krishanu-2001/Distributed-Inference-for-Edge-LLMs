...
class TokenizerCommunicatorMixin:
    ...
    def init_communicators(self: TokenizerManager, server_args: ServerArgs):
        ...
        self.read_radix_tree_communicator = _Communicator(
            self.send_to_scheduler, server_args.dp_size
        )
    ...
    def _get_communicator_dispatcher(self: TokenizerManager):
        return TypeBasedDispatcher(
            [
                ...
                (
                    ReadRadixTreeReqOutput,
                    self.read_radix_tree_communicator.handle_recv,
                ),
                (
                    SetInternalStateReqOutput,
    ...
    async def read_radix_tree(
        self: TokenizerManager, obj: ReadRadixTreeReq
    ) -> List[Dict[Any, Any]]:
        self.auto_create_handle_loop()
        responses: List[ReadRadixTreeReqOutput] = (
            await self.read_radix_tree_communicator(obj)
        )
        return [res.radix_tree for res in responses]

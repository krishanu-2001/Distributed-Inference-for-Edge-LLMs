...
@app.get("/debug/get_radix_tree")
async def read_normal_radix_tree(
    max_nodes: int = Query(default=1000, ge=1, le=100000),
    include_values: bool = Query(default=False),
):
    # return {"file": "hellob"}
    obj = ReadRadixTreeReq(max_nodes=max_nodes, include_values=include_values)
    # return {"file": "hello"}
    return await _global_state.tokenizer_manager.read_radix_tree(obj)


@app.get("/debug/radix_tree")
@auth_level(AuthLevel.ADMIN_OPTIONAL)

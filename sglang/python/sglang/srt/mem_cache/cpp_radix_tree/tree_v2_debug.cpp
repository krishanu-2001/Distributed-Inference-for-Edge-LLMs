...
std::string RadixTree::debug_dump() const {
  std::ostringstream os;
  try {
    m_impl->debug_print(os);
  } catch (const std::exception& e) {
    os << "RadixTree::debug_dump captured exception: " << e.what() << std::endl;
  } catch (...) {
    os << "RadixTree::debug_dump captured unknown exception." << std::endl;
  }
  return os.str();
}
